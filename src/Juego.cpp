...
void Juego::bucleJuego()
{
	...
	window.create(sf::VideoMode(800.0f * screenScalingFactor, 600.0f * screenScalingFactor), "Family Billiards");
	...
	while (window.isOpen())
	{
		...
		while (window.pollEvent(event))
		{
			...
			else if (event.type == sf::Event::KeyPressed)
			{
				...
				else if (estado == 1 && (event.key.code == sf::Keyboard::Up || event.key.code == sf::Keyboard::Down || event.key.code == sf::Keyboard::Left || event.key.code == sf::Keyboard::Right))
				{
		...
		for (unsigned int i = 0; i < bolas.size(); i++)
		{
			if (!bolas[i].getCaida() || bolas[i].getAnimado())
				bolas[i].Render(window, percentick);
		}

		for (unsigned int i = 0; i < barra.size(); i++)
		{
			barra[i].Render(window, percentick);
		}

		if (estado == 0 || estado == 1)
		{
			Jugador::Instance()->Render(window, percentick);
		}

		if ((estado == 2 && palo.getTerminado()) || estado == 3)
		{
			Jugador::Instance()->RenderPoints(window);
			abaco.Render(window, percentick);
		}
		...
		if (debug)
		{
			...
			{
				window.draw(troneras[i]);
			}
			for (unsigned int i = 0; i < paredes.size(); i++)
			{
				window.draw(paredes[i]);
			}
		}

		if (estado == 0 || estado == 1 || (estado == 2 && !palo.getTerminado()))
		{
			palo.Render(window, percentick);

			window.setView(mapa);
			window.draw(circulo);
			window.draw(circulo2);
			window.draw(circulo3);
			window.draw(circulo4);
			Mapa::Instance()->Render(window, bolas);
		}
